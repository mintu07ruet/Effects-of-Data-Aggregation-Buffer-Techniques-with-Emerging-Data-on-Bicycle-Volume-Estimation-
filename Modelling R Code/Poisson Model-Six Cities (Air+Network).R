...
# Run Euclidian+Network buffer model
# Buffer Size =all
# run the model
...
set.seed(seed)
pm1.cv <- train(as.integer(AADBT) ~  stv_c_adb+log_stv_stl
                +                     min_dist_to_college
                +                              stv_nc_adb
                +                            Footway_otm
...
set.seed(seed)
pm2.cv <- train(as.integer(AADBT) ~   stv_c_adb+log_stv_stl
                +                           tertiary_binary
                +                                stv_nc_adb
                +                        Distance_to_CBD_mi
                +                             Bus.Stops_qm
...
set.seed(seed)
pm3.cv <- train(as.integer(AADBT) ~      stv_c_adb+log_stv_stl
                +                           tertiary_binary
                +                        Distance.to.forest
                +                                stv_nc_adb
                +                        Student.Access_hm
...
set.seed(seed)
pm4.cv <- train(as.integer(AADBT) ~           stv_c_adb+log_stv_stl+
                +                           tertiary_binary
...
set.seed(seed)
pm0.cv <- train(as.integer(AADBT) ~     stv_c_adb
                +                       tertiary_binary
               +                        University_tm
...
#                +               Distance.to.Grass
 #               +                Bike.Commuter_tm
                +                     Cycleway_tm
 #               +        cycleway_lane_all_tm_net
#                +       cycleway_track_all_tm_net
                +               pct_white_tm_net
#                +             sep_bikeway_binary
#                +                 BikeFac_binary
                +             Distance_to_CBD_mi
...
# all mile buffer model
set.seed(seed)
pm0.cv <- train(as.integer(AADBT) ~ Bicycle.Parking_otm
#                +                Cycleway_qm
#                +               University_hm
...
set.seed(seed)
pm2.cv <- train(as.integer(AADBT) ~  
#                                         Distance.to.Park
                                             log_stv_c_adb
                +                       Bicycle.Parking_qm
                +                            University_qm
#                +                    Number.of.jobs_qm_net
#                +                             slope_qm_net
#+pct_white_qm
...
pm4.cv <- train(as.integer(AADBT) ~ Distance.to.Park
                ...
                +                         log_stv_nc_adb
                +                            log_stl_raw
                +    Percentage.of.Bike.Commuter_om_net
...
set.seed(seed)
pm0.cv <- train(as.integer(AADBT) ~ Park.Area_otm_net
                +     pct_at_least_college_education_qm_net
                +              pct_African_American_om_net
                +                      sep_onstreet_binary
                +                          log_stv_nc_adb
                +                              log_stl_raw
...
set.seed(seed)
pm1.cv <- train(as.integer(AADBT) ~pct_at_least_college_education_otm
                +                           Path_otm_net
                +                    sep_onstreet_binary
                +                           log_stl_raw
                ,
...
set.seed(seed)
pm3.cv <- train(as.integer(AADBT) ~stv_nc_adb+log_stl_raw
                +                   sep_onstreet_binary
                +                           log_stl_raw
                +                        School_hm_net
                ,
...
set.seed(seed)
pm5.cv <- train(as.integer(AADBT) ~pct_African_American_om_net
                +                     Distance.to.Park
                +                  sep_onstreet_binary
                +                       log_stv_c_adb
                ,
...
set.seed(seed)
pm0.cv <- train(as.integer(AADBT) ~Median_HH_income_tm_net
                +                      sep_bikeway_otm_net
                +                            log_stv_c_adb
...
set.seed(seed)
pm1.cv <- train(as.integer(AADBT) ~slope_otm
#                +               Forest.Area_otm_net
...
#                +                         University_hm_net
#               +                             bridge_hm_net
#                +              pct_African_American_hm_net
#                +    pct_at_least_college_education_hm_net
#                +                  Median_HH_income_hm_net
#                +                             slope_hm_net
#                +                               BikeFac_hm
#                +                            Park_acres_hm
                ,
...
pm3.cv <- train(as.integer(AADBT) ~stv_nc_adb
                +                                log_stl_raw
#                +               Intersection_Density_tfm_net
#                +                        Retail.Area_tfm_net
#                +                          Park.Area_tfm_net
#                +                    Bicycle.Parking_tfm_net
                +                            college_tfm_net
#                +                           Tertiary_tfm_net
#                +                             bridge_tfm_net
...
set.seed(seed)
pm4.cv <- train(as.integer(AADBT) ~Number.of.jobs_om_net
...
plot_regions(pm4.cv, bo_data, title="Boulder Best Model with 1.0 mile Air and Network Buffers", scales="fixed")  # scales="free" is an option
plot_all(pm4.cv, bo_data, title="Boulder Model for all Buffers", label_outliers=NA)

# Boulder 1.5 mile buffer model
set.seed(seed)
pm5.cv <- train(as.integer(AADBT) ~log_stv_nc_adb
                +Median_HH_income_ohm_net
...
set.seed(seed)
pm6.cv <- train(as.integer(AADBT) ~pct_at_least_college_education_tm_net
...
set.seed(seed)
pm2.cv <- train(as.integer(AADBT) ~ Park_acres_qm
