...
# Network Model for all buffers for six cities
...
all_data$Strava_c_adb

# General statistics
select_data=c('AADBT','stv_c_adb','stv_nc_adb','log_stv_stl','tertiary_binary','Path_hm_net','Distance.to.forest','OSM_node_Density_hm_net','pct_at_least_college_education_hm_net','Student.Access_hm')
gdata=all_data[,select_data]

summary(gdata)
sapply(gdata, sd, na.rm = TRUE)
...
pm8.cv <- train(as.integer(AADBT) ~   stv_c_adb
                ...
                +                          Footway_otm_net
                +                         Tertiary_tfm_net
                +                   Student.Access_tfm_net
                +                  OSM_node_Density_tm_net
...
pm2.cv <- train(as.integer(AADBT) ~   stv_c_adb
               ...
               +                               log_stl_raw
                +                         Bus.Stops_qm_net
                +                  OSM_node_Density_qm_net
                +                              Path_qm_net
                +                           Footway_qm_net
                +    pct_at_least_college_education_qm_net
...
pm5.cv <- train(as.integer(AADBT) ~             stv_c_adb
               ...
               +                    Student.Access_om_net
                +   pct_at_least_college_education_om_net
...
pm6.cv <- train(as.integer(AADBT) ~            stv_c_adb
                ...
                +    pct_at_least_college_education_ohm_net
                ,
                data = all_data, method = "glm",family='poisson',
                trControl = train_control5)
#family=sm.families.NegativeBinomial(link=sm.families.links.identity)
...
set.seed(seed)
pm7.cv <- train(as.integer(AADBT) ~             tertiary_binary
                +                        Distance.to.forest
                +                                stv_nc_adb
                +                               log_stv_stl
                +                    Student.Access_tm_net
                +    pct_at_least_college_education_tm_net
                ,
                data = all_data, method = "glm",family='poisson',
                trControl = train_control5)
...
'HH_density_otm')
spdata=p_data[,select_data]

summary(spdata)
sapply(spdata, sd, na.rm = TRUE)



# all mile buffer model
set.seed(seed)
pm0.cv <- train(as.integer(AADBT) ~    min_dist_to_college
                +                           log_stv_c_adb
                +                             log_stl_raw
                +     Percentage.of.Bike.Commuter_ohm_net
#                +                      pct_white_ohm_net
                +                Median_HH_income_tm_net
...
plot_regions(pm1.cv, p_data, title="Portland Model for all  Buffers", scales="fixed")  # scales="free" is an option
...
pm7.cv <- train(as.integer(AADBT) ~    stv_c_adb
                ...
                +           Distance.to.Residential.Area
                +                    sep_bikeway_binary
                +    Percentage.of.Bike.Commuter_tm_net
#                +                      pct_white_tm_net
#                +               Median_HH_income_tm_net
...
# General statistics
select_data=c('AADBT','log_stv_c_adb',
'stl_raw',
'Intersection_Density_otm_net',
'University_otm_net',
'Bicycle.Parking_otm_net',
'bridge_otm_net',
'Student.Access_otm')
seudata=e_data[,select_data]

summary(seudata)
sapply(seudata, sd, na.rm = TRUE)
...
pm0.cv <- train(as.integer(AADBT) ~ log_stv_c_adb
                +                    Bicycle.Parking_otm_net
                +                         University_otm_net
                +    pct_at_least_college_education_otm_net
                +                   Bicycle.Parking_ohm_net
                ,
...
set.seed(seed)
pm1.cv <- train(as.integer(AADBT) ~   stl_raw
                +                            tertiary_binary
                +                                 stv_nc_adb
                +               Intersection_Density_otm_net
                +                        University_otm_net
...
set.seed(seed)
pm2.cv <- train(as.integer(AADBT) ~  Distance.to.Park
                +                  Distance.to.Water.Center
                +                            log_stv_nc_adb
                +                   Bicycle.Parking_qm_net
                +                        University_qm_net
                +    pct_at_least_college_education_qm_net
                ,
                data = e_data, method = "glm",family='poisson',
...
set.seed(seed)
pm5.cv <- train(as.integer(AADBT) ~   tertiary_binary
                +                Distance.to.Park
                +        Distance.to.Water.Center
                +                  log_stv_nc_adb
                +                    log_stl_raw
                +             University_ohm_net
                +     population_density_ohm_net
                ,
                data = e_data, method = "glm",family='poisson',
                trControl = train_control5)
#family=sm.families.NegativeBinomial(link=sm.families.links.identity)
...
be_data$log_stv_nc_adb
# General statistics
select_data=c('AADBT','stv_nc_adb','log_stl_raw','sep_onstreet_binary',  'Bus.Stops_qm' )
sbdata=be_data[,select_data]

summary(sbdata)
sapply(sbdata, sd, na.rm = TRUE)
...
set.seed(seed)
pm2.cv <- train(as.integer(AADBT) ~min_dist_to_college
                +                       sep_onstreet_binary
                +                             log_stv_c_adb
                +                               log_stl_raw
                +                    Bicycle.Parking_qm_net
...
plot_regions(pm2.cv, be_data, title="Bend Best Model with 0.25 mile Air and Network Buffers", scales="fixed")  # scales="free" is an option
...
# General statistics
select_data=c('AADBT','stv_nc_adb',
              'log_stl_raw',
              'Commercial.Area_qm',
              'School_qm',
...
sdadata=da_data[,select_data]

summary(sdadata)
sapply(sdadata, sd, na.rm = TRUE)



# Dallas all mile buffer model
set.seed(seed)
pm0.cv <- train(as.integer(AADBT) ~log_stl_raw
                +                                stv_nc_adb
                +                          pct_white_om_net
                ,
...
set.seed(seed)
pm1.cv <- train(as.integer(AADBT) ~Distance.to.Park
                +                        stv_nc_adb
                +                       log_stl_raw
...
plot_regions(pm2.cv, da_data, title="Bend Model for all  Buffers", scales="fixed")  # scales="free" is an option
plot_all(pm2.cv, da_data, title="Bend Model for all Buffers", label_outliers=NA)

# Dallas 0.50 mile buffer model
set.seed(seed)
pm3.cv <- train(as.integer(AADBT) ~Distance.to.Park
                +                                stv_nc_adb
                +                            BikeFac_binary
                +                               log_stl_raw
                +                        Grass.Area_hm_net
...
sbodata=bo_data[,select_data]

summary(sbodata)
sapply(sbodata, sd, na.rm = TRUE)
...
plot_regions(pm4.cv, bo_data, title="Boulder Best Model with 1.0-mile Network Buffer", scales="fixed")  # scales="free" is an option
plot_all(pm4.cv, bo_data, title="Boulder Model for all Buffers", label_outliers=NA)

# Boulder 1.5 mile buffer model
set.seed(seed)
pm5.cv <- train(as.integer(AADBT) ~ 
Tertiary_ohm
#                +               pct_African_American_ohm
#                +     pct_at_least_college_education_ohm
                +                   Median_HH_income_ohm
#                +                              slope_ohm
                +                        log_stv_nc_adb
#                +                        Park_acres_ohm
...
# General statistics
select_data=c('AADBT','BikeFac_binary',
              'arterial_binary')
schdata=ch_data[,select_data]

summary(schdata)
sapply(schdata, sd, na.rm = TRUE)
...
plot_regions(pm0.cv, ch_data, title="Charlotte Best Model without any Buffer Variables", scales="fixed")  # scales="free" is an option
plot_all(pm0.cv, ch_data, title="Boulder Model for all Buffers", label_outliers=NA)

# Charlotte 0.1 mile buffer model
set.seed(seed)
pm1.cv <- train(as.integer(AADBT) ~BikeFac_binary+arterial_binary
                ,
                data = ch_data, method = "glm",family='poisson',
                trControl = train_control5)
#family=sm.families.NegativeBinomial(link=sm.families.links.identity)
...
set.seed(seed)
pm2.cv <- train(as.integer(AADBT) ~BikeFac_binary
               + arterial_binary
...
plot_regions(pm2.cv, ch_data, title="Charlotte Best Model with 0.25 mile Air and Network Buffers", scales="fixed")  # scales="free" is an option
plot_all(pm2.cv, ch_data, title="Boulder Model for all Buffers", label_outliers=NA)

# Charlotte 0.50 mile buffer model
set.seed(seed)
pm3.cv <- train(as.integer(AADBT) ~BikeFac_binary+
                  +arterial_binary
                ,
                data = ch_data, method = "glm",family='poisson',
                trControl = train_control5)
...
set.seed(seed)
pm4.cv <- train(as.integer(AADBT) ~ BikeFac_binary+
                  +arterial_binary
